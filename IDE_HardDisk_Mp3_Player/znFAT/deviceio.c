...
//#include "ch375.h" //存储设备驱动头文件
#include "ATAPI6.h"	 //硬盘驱动头文件
...
// res=SdCard_Init(); 
//	res=CH375_InitDisk(); //必须确保存储设备驱动程序是完美可用的，否则出现问题可不是文件系统层的原因。
	res=HDD_Init();	  //存储设备初始化函数和znFAT挂接 第一项，一共6项。
 if(res) err|=0X01;
...
UINT8 znFAT_Device_Read_Sector(UINT32 addr,UINT8 *buffer)
{
 ...
 {
  case 0:
	    		 while(ReadSector(addr,buffer));	//读取一个扇区函数和znFAT挂接 第二项，一共6项。
				//	 while(CH375_Read_Sector(addr,buffer));
		     break;						  //必须确保存储设备驱动程序是完美可用的，否则出现问题可不是文件系统层的原因。
  //case 1:
...
UINT8 znFAT_Device_Write_Sector(UINT32 addr,UINT8 *buffer) 
{
 ...
 {
  case 0:
	      		 while(WriteSector(addr,buffer));	//写一个扇区函数和znFAT挂接 第三项，一共6项。
				//	while(CH375_Write_Sector(addr,buffer));	  //存储设备和znFAT挂接，就是这么简单！
		     break;				//必须确保存储设备驱动程序是完美可用的，否则出现问题可不是文件系统层的原因。
  //case 1:
 ...
 if(0==nsec) return 0;
  ...
  switch(Dev_No)
  {
   case 0:
          ...
          {	
		   while(ReadSector(addr+i,buffer));  	//读n个扇区函数和znFAT挂接 第4项，一共6项。
          // while(SD1_Read_Sector(addr+i,buffer));
           buffer+=512;
          }
          break;
  // case 1:
 ...
 if(0==nsec) return 0;
  ...
  switch(Dev_No)
  {
   case 0:
          for(i=0;i<nsec;i++)
          {
		  while(WriteSector(addr+i,buffer));  	//写n个扇区函数和znFAT挂接 第5项，一共6项。
          // while(SD1_Write_Sector(addr+i,buffer));
           buffer+=512;
          }
		  break;
   //case 1:
...
UINT8 znFAT_Device_Clear_nSector(UINT32 nsec,UINT32 addr)
{
  ...
  switch(Dev_No)
  {
   case 0:
          for(i=0;i<nsec;i++)
          {
		   while(WriteSector(addr+i,znFAT_Buffer));	 //写1个扇区函数和znFAT挂接 第5项，一共6项。
		   										//注意这里的第二个参数是znFAT的内部缓冲区 znFAT_Buffer
         //  while(SD1_Write_Sector(addr+i,znFAT_Buffer));
          }
          break;
   //case 1:
   //       for(i=0;i<nsec;i++)
